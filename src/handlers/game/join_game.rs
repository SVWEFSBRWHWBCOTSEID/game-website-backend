...
use crate::helpers::general::{gen_guest_id, get_user_with_relations, get_username};
...
pub async fn join_game(
    ...
    let user = if game.with_friend {
        CreateUserReq {
            username: gen_guest_id(&client).await,
            guest: true,
            password: "".to_string(),
            preferences: None,
        ...
        get_user_with_relations(&client, &username.clone()).await?
    };

    let perf = user.perfs().unwrap().iter().find(|p| p.game_key == game.game_key).unwrap();
    join_game_util(&client, &game, game.first_username.is_none(), user.username.clone(), perf.rating as i32, perf.prov, &broadcaster, &player_stats).await?;
